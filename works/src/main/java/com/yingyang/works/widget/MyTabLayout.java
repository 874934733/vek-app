package com.yingyang.works.widget;/*
 ...
 *
 * <p>You can also add items to TabLayout in your layout through the use of {@link}. An
...
public class MyTabLayout extends HorizontalScrollView {
    ...
    @Dimension(unit = Dimension.DP)
    private static final int MIN_INDICATOR_WIDTH = 48;
     ...
     * MyTabLayout}. This will only take effect if the indicator height is set via the custom indicator
     ...
     * width of the {@link MyTabLayout}. This will disregard {@code tabIndicatorHeight} and the
    ...
    public static final int INDICATOR_GRAVITY_MACH_PARENT = 4;
    ...
    //默认字体
    private static int textSizeNormal = 18;
    private int textSizeSelected = 18;
...
//        scrollableTabMinWidth = res.getDimensionPixelSize(R.dimen.design_tab_scrollable_min_width);
    ...
    /**
     * Set the indicator gravity used to align the tab selection indicator in the {@link MyTabLayout}.
     ...
     * indicator height and stretch across the entire height and width of the {@link MyTabLayout}. This
    ...
    /**
     * Set whether this {@link MyTabLayout} will have an unbounded ripple effect or if ripple will be
    ...
    /**
     * Returns whether this {@link MyTabLayout} has an unbounded ripple effect, or if ripple is bound to
     * the tab item size.
     *
     * @attr ref com.google.android.material.R.styleable#TabLayout_tabUnboundedRipple
...
//                addTab(newTab().setText(pagerAdapter.getPageTitle(i)), false);
                Tab myTab = newTab().setText(pagerAdapter.getPageTitle(i));
                TextView myTextView = (TextView) myTab.customView;
                myTextView.setTextSize(textSizeNormal);
                addTab(myTab, false);
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // height
        @SuppressLint("RestrictedApi") final int idealHeight =
                (int)
    ...
    public void selectTab(@Nullable final Tab tab, boolean updateIndicator) {
        ...
        TextView txtView = (TextView) tab.customView;
        txtView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, isZoom ? textSizeSelected : textSizeNormal);
        txtView.setTypeface(Typeface.defaultFromStyle(isZoom ? Typeface.BOLD : Typeface.NORMAL));
...
//                    this, tabPaddingStart, tabPaddingTop, tabPaddingEnd, tabPaddingBottom);
        ...
        protected void drawableStateChanged() {
            ...
            if (changed) {
                invalidate();
                MyTabLayout.this.invalidate(); // Invalidate TabLayout, which draws mBaseBackgroundDrawable
            }
        }
    ...
    private class SlidingTabIndicator extends LinearLayout {
        ...
        public void draw(Canvas canvas) {
            ...
            int iWidth = indicatorRight - indicatorLeft;

            switch (tabIndicatorGravity) {
                case INDICATOR_GRAVITY_BOTTOM:
                case INDICATOR_GRAVITY_MACH_PARENT:
...
//                selectedIndicator.setBounds(indicatorLeft , indicatorTop, indicatorRight , indicatorBottom);
                /*int strokeWidth = 5;
                int roundRadius = 15;
                int strokeColor = Color.parseColor("#2E3134");
                selectedIndicator = new GradientDrawable();
                selectedIndicator.setColor(Color.parseColor("#ff0000"));
                selectedIndicator.setCornerRadius(roundRadius);
                selectedIndicator.setStroke(strokeWidth,strokeColor);

                int iWidth = indicatorRight - indicatorLeft;
//                selectedIndicator.setBounds(indicatorLeft , indicatorTop, indicatorRight , indicatorBottom);
                //新建矩形
                RectF r2 = new RectF();
                r2.left = indicatorLeft +  iWidth/2 - 10 ;
                r2.right = indicatorLeft +  iWidth/2 + 10;
                r2.top = indicatorTop ;
                r2.bottom = indicatorBottom;*/
//                selectedIndicator.drawRoundRect( indicatorLeft +  iWidth/2 - 10 ,indicatorTop , indicatorLeft +  iWidth/2 + 10 , indicatorBottom);
//                selectedIndicator.setBounds();
//                selectedIndicator.set( indicatorLeft +  iWidth/2 - 10 ,indicatorTop , indicatorLeft +  iWidth/2 + 10 , indicatorBottom);
                //5.6
//                selectedIndicator.setBounds( indicatorLeft +  iWidth/2 - 20 ,indicatorTop , indicatorLeft +  iWidth/2 + 20 , indicatorBottom);
                if(tabIndicatorGravity == INDICATOR_GRAVITY_MACH_PARENT){
                    selectedIndicator.setBounds(indicatorLeft, indicatorTop, indicatorLeft + getWidth() / 2, indicatorBottom);
                }else{
                    selectedIndicator.setBounds(indicatorLeft + iWidth / 2 - 40, indicatorTop, indicatorLeft + iWidth / 2 + 40, indicatorBottom);
                }
    ...
    public static class ViewPagerOnTabSelectedListener implements OnTabSelectedListener {
        ...
        @Override
        public void onTabUnselected(Tab tab) {
            TextView textView = (TextView) tab.customView;
            textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, textSizeNormal);
